...
### Fixed
- A path `parse_url()` could not read — `/notes:80`, `/x:1/y` — or read as a host — `//evil.com/` — was routed as
  `/` and answered by the home page with a 200. The Kernel now splits the query string off at the first `?` and
  routes what is before it; those 404.
- `/notes/` matched `/notes/{id}` with an `id` of `''`. An empty segment no longer satisfies a parameter, so a
  trailing slash on a collection 404s instead of reaching Show.
- A segment that percent-decodes to a NUL byte (`%00`) matches no route, rather than reaching the application.
- `Response` refuses a header carrying a NUL byte, as it already refused a newline; PHP dropped it with a warning.
- `Log` flattened newlines after its "cannot create log directory" fallback had already passed the raw message to
  `error_log()`. The fallback gets one line too.

## [0.14.0] — 2026-09-20

### Changed
- Path segments are percent-decoded, one at a time, in the Router: `/posts/caf%C3%A9` captures `café`, `/gre%65t`
  matches `/greet`, and an encoded slash inside a segment stays inside it. Nothing decoded before; an Action that
  decoded a parameter itself now decodes twice and should stop.
- `Response` refuses a header name or value carrying a newline with an `InvalidArgumentException`, instead of PHP
  warning and silently dropping the header.
- `Session` has two timeouts, both constructor arguments: `idleTimeout` (default 30 minutes) and `lifetime`
  (default 12 hours). One constant of 30 minutes used to apply to both, signing out active visitors at minute 31.
  The `Session::TIMEOUT` constant is gone.
- `Log` writes one line per entry: newlines in a message become spaces, so a message that echoes visitor input cannot
  forge an entry.

## [0.13.0] — 2026-09-20

### Added
- `Middleware\SecurityHeaders`: `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY` and
  `Referrer-Policy: strict-origin-when-cross-origin` on every response, error pages included. Takes the list to send,
  so an application can add a Content-Security-Policy or change the frame policy. None is shipped by default; the
  skeleton composes it.
- `new Session(trustForwardedProto: true)` believes `X-Forwarded-Proto: https` when setting the cookie's `Secure`
  flag, for deployments behind a TLS-terminating proxy. Off by default, because any client can send the header.

### Changed
- Sessions start in strict mode (`session.use_strict_mode`), so a session ID the client invented is refused rather
  than initialised.
- `Session::regenerateId()` also drops the CSRF token, so a token issued before a login does not authorise writes
  after it.

## [0.12.1] — 2026-09-17

### Fixed
- The console told you to run `tether help <command>`, `tether routes` and `tether make:action`, and every `Usage:`
  line and `context`'s `generate` hints said the same — none of which is a command on anyone's PATH. All of it now
  says `php tether …`, the way the docs always have.
...
[Unreleased]: https://github.com/Dillonsmart/tetherphp-core/compare/v0.14.1...HEAD
[0.14.1]: https://github.com/Dillonsmart/tetherphp-core/compare/v0.14.0...v0.14.1
[0.14.0]: https://github.com/Dillonsmart/tetherphp-core/compare/v0.13.0...v0.14.0
[0.13.0]: https://github.com/Dillonsmart/tetherphp-core/compare/v0.12.1...v0.13.0
[0.12.1]: https://github.com/Dillonsmart/tetherphp-core/compare/v0.12.0...v0.12.1
